...
namespace JEngine {
	namespace Systems {
		...
		void CollisionSystem::update(float dt) {
			...
			for (auto it = objects->components[Core::Component::COLLISION].begin(); it != objects->components[Core::Component::COLLISION].end(); it++) {
				auto jt = it;
				
				for (jt++; jt != objects->components[Core::Component::COLLISION].end(); jt++) {
					Components::Collision* c1 = (Components::Collision*) (*it);
					Components::Collision* c2 = (Components::Collision*) (*jt);

					Core::Entity* e1 = c1->owner;
					Core::Entity* e2 = c2->owner;

					Components::Shape* s1 = (Components::Shape*) (e1->components[Core::Component::SHAPE]);
					Components::Shape* s2 = (Components::Shape*) (e2->components[Core::Component::SHAPE]);
